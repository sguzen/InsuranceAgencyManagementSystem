...
namespace IAMS.Application.Features.Policies.Queries.GetRevenueByMonth
{
    ...
    public async Task<Result<Dictionary<string, decimal>>> Handle(GetRevenueByMonthQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return Result<Dictionary<string, decimal>>.Unauthorized("Kiracı bağlamı bulunamadı");
            }

            var tenantId = _currentTenantService.TenantId.Value;
            var revenueByMonth = await _unitOfWork.Policies.GetRevenueByMonthAsync(tenantId, request.Months);
