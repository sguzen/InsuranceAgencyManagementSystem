...
namespace IAMS.Persistence.Repositories
{
    public class PolicyPaymentRepository : Repository<PolicyPayment>, IPolicyPaymentRepository
    {
        ...
        public async Task<PolicyPaymentDto?> GetByIdDtoAsync(int id)
        {
            return await _dbSet
                ...
                .Select(pp => new PolicyPaymentDto
                {
                    ...
                    CurrencySymbol = pp.Policy.Currency.Symbol,
                    CreatedOn = pp.CreatedOn
                })
                .FirstOrDefaultAsync();
        }

        public async Task<List<PolicyPaymentListDto>> GetPaymentsByPolicyIdDtoAsync(int policyId)
        {
            return await _dbSet
                .Where(pp => pp.PolicyId == policyId && !pp.IsDeleted)
                .OrderByDescending(pp => pp.PaymentDate)
                .Select(pp => new PolicyPaymentListDto
                {
                    ...
                    PolicyNumber = pp.Policy.PolicyNumber,
                    CustomerName = pp.Policy.Customer.FirstName + " " + pp.Policy.Customer.LastName,
                    InsuranceCompanyName = pp.Policy.InsuranceCompany.Name,
                    Amount = pp.Amount,
                    PaymentDate = pp.PaymentDate,
                    DueDate = pp.DueDate,
                    Status = pp.Status,
                    CurrencyCode = pp.Policy.Currency.Code,
                    Reference = pp.Reference
                })
                .ToListAsync();
        }

        public async Task<List<PolicyPaymentListDto>> GetOverduePaymentsDtoAsync()
        {
            var today = DateTime.Today;
            return await _dbSet
                .Where(pp => !pp.IsDeleted &&
        ...
        public async Task<List<PolicyPaymentListDto>> GetPaymentsByDateRangeDtoAsync(DateTime fromDate, DateTime toDate)
        {
            return await _dbSet
                .Where(pp => !pp.IsDeleted &&
                            pp.PaymentDate >= fromDate &&
        ...
        public async Task<List<PolicyPaymentListDto>> GetPaymentsDueThisMonthDtoAsync()
        {
            var today = DateTime.Today;
            var startOfMonth = new DateTime(today.Year, today.Month, 1);
            var endOfMonth = startOfMonth.AddMonths(1).AddDays(-1);
